...
// Sony standard 1/3-stop shutter speeds (seconds)
const SHUTTER_VALS = [
    ...
    1/8, 1/6, 1/5, 1/4, 1/3, 0.4, 0.5, 0.6, 0.8,
    1, 1.3, 1.6, 2, 2.5, 3, 4, 5, 6, 8, 10, 13, 15, 20, 25, 30
];

// Sony standard 1/3-stop ISO values
const ISO_VALS = [
...
// Sony standard 1/3-stop aperture f-numbers
const APERTURE_VALS = [
...
let _loupeUserVisible = false;          // controlled by 🔍 toggle button — off by default
let relay1On = false;
let relay2On = false;
let motionScripts = [];
...
const els = {
    ...
    expManualControls: document.getElementById('exp_manual_controls'),
    expModeAutoLbl:   document.getElementById('exp_mode_auto_lbl'),
    expModeManualLbl: document.getElementById('exp_mode_manual_lbl'),
    expModeCameraLbl: document.getElementById('exp_mode_camera_lbl'),
    apertureSlider:   document.getElementById('aperture_slider'),
    apertureLabel:    document.getElementById('aperture_label'),
    apertureRow:      document.getElementById('aperture_row'),
    camSettingsReadout: document.getElementById('cam_settings_readout'),
...
function initSliders() {
    ...
    if (els.apertureSlider) {
        const apDefault = APERTURE_VALS.findIndex(v => Math.abs(v - 2.8) < 0.05);
        els.apertureSlider.max = APERTURE_VALS.length - 1;
        els.apertureSlider.value = apDefault >= 0 ? apDefault : 9;
        if (els.apertureLabel) els.apertureLabel.innerText = `f/${APERTURE_VALS[els.apertureSlider.value]}`;
    }
}
...
function onApertureSlider(val) {
    const f = APERTURE_VALS[parseInt(val)];
    if (els.apertureLabel) els.apertureLabel.innerText = `f/${f}`;
    sendPicamSettings();
}

// ─── EXPOSURE MODE ────────────────────────────────────────────────────────────
let _expMode = 'auto';   // 'auto' | 'manual' | 'camera'

function _expModeApplyUI(mode) {
    const lbls = {
        auto:   els.expModeAutoLbl,
        manual: els.expModeManualLbl,
        camera: els.expModeCameraLbl,
    };
    const active = { background: 'var(--accent-gold)', color: '#000', fontWeight: '600' };
    const idle   = { background: 'var(--surface)',     color: 'var(--text-dim)', fontWeight: 'normal' };
    Object.entries(lbls).forEach(([k, el]) => {
        if (!el) return;
        Object.assign(el.style, k === mode ? active : idle);
    });
    if (els.expManualControls)
        els.expManualControls.style.display = mode === 'manual' ? '' : 'none';
    if (els.camSettingsReadout)
        els.camSettingsReadout.style.display = mode === 'camera' ? '' : 'none';
    // Keep hidden checkboxes in sync for any legacy code that reads them
    if (els.aeToggle)  els.aeToggle.checked  = mode === 'auto';
    if (els.awbToggle) els.awbToggle.checked = mode === 'auto';
}

function onExpModeChange(mode) {
    _expMode = mode;
    _expModeApplyUI(mode);
    sendPicamSettings();
    if (mode === 'camera') {
        sendCmd('get_camera_settings');
    }
}

function sendPicamSettings() {
    const s      = SHUTTER_VALS[parseInt(els.shutterSlider?.value || 0)];
    const iso    = Math.round(ISO_VALS[parseInt(els.isoSlider?.value || 0)]);
    const kelvin = parseInt(els.wbSlider?.value || 5500);
    const ap     = els.apertureSlider ? APERTURE_VALS[parseInt(els.apertureSlider.value)] : null;
    const ae     = _expMode === 'auto';
    const awb    = _expMode === 'auto';
    const cmd = { ae, awb, shutter_s: s, iso, kelvin, exp_mode: _expMode };
    if (ap !== null) cmd.aperture_f = ap;
    sendCmd('set_picam_settings', cmd);
}
...
function _applyModeUI(mode) {
    ...
    // PiP preview: start in cinematic mode, stop otherwise
    if (isCinematic) {
        startPiPRefresh();
    } else {
        stopPiPRefresh();
...
function calcDurationFrames() {
    ...
    if (hgOn && ws && ws.readyState === WebSocket.OPEN) {
        out.innerText = `Calculating…  (${(secs / 3600).toFixed(1)} hrs)`;
        sendCmd('estimate_hg_frames', { start_iso: st, end_iso: et });
    } else {
...
function getSeqConfig() {
    ...
    if (seqMode === 'frames') {
        ...
        if (hgOn) {
            // Use server-estimated total (synced into total_frames by hg_frame_estimate handler)
            // Fall back to flat calculation only if estimate hasn't arrived yet
            const estimated = parseInt(document.getElementById('total_frames')?.value);
            interval = parseFloat(document.getElementById('hg_int_day')?.value) || 5;
            total_frames = estimated > 0 ? estimated : Math.max(1, Math.floor(secs / interval));
            log(`⏱ HG frame estimate: ${total_frames} frames across ${(secs/3600).toFixed(1)} hrs`);
        } else {
    ...
    if (seqMode === 'duration') {
        ...
        const etVal = document.getElementById('seq_end_time')?.value;
        if (stVal) {
...
function setRunState(running) {
    ...
    // Macro start / flats buttons
    const macroStartBtn = document.getElementById('macroStartBtn');
    const macroFlatsBtn = document.getElementById('macroFlatsBtn');
    [macroStartBtn, macroFlatsBtn].forEach(btn => {
        if (!btn) return;
...
// ─── MOTOR DRIVER CONFIG (MKS servo42D / steps-per-unit) ─────────────────────
const PAN_GEAR_RATIO  = 15;
const TILT_GEAR_RATIO = 30;

function _motorConfigInputs() {
    return {
        pan_steps_rev:    parseFloat(document.getElementById('motor_pan_steps_rev')?.value  || 1600),
        tilt_steps_rev:   parseFloat(document.getElementById('motor_tilt_steps_rev')?.value || 1600),
        slider_steps_mm:  parseFloat(document.getElementById('motor_slider_steps_mm')?.value || 50.0),
        focus_steps_mm:   parseFloat(document.getElementById('motor_focus_steps_mm')?.value  || 800.0),
    };
}

function _updateMotorDerivedLabels(cfg) {
    const panDeg  = (cfg.pan_steps_rev  * PAN_GEAR_RATIO)  / 360;
    const tiltDeg = (cfg.tilt_steps_rev * TILT_GEAR_RATIO) / 360;
    const el = id => document.getElementById(id);
    if (el('motor_pan_steps_deg'))  el('motor_pan_steps_deg').textContent  = panDeg.toFixed(2)  + ' steps/°';
    if (el('motor_tilt_steps_deg')) el('motor_tilt_steps_deg').textContent = tiltDeg.toFixed(2) + ' steps/°';
}

function sendMotorConfig() {
    const cfg = _motorConfigInputs();
    _updateMotorDerivedLabels(cfg);
    fetch('/api/motor-config', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify(cfg),
    }).then(r => r.json()).then(d => {
        if (d.ok) console.log('Motor config applied:', d);
    }).catch(e => console.warn('motor-config failed:', e));
}

function _applyMotorConfigFromState(state) {
    if (!state.motor_config) return;
    const cfg = state.motor_config;
    const el = id => document.getElementById(id);
    if (cfg.pan_steps_rev   != null && el('motor_pan_steps_rev'))   el('motor_pan_steps_rev').value   = cfg.pan_steps_rev;
    if (cfg.tilt_steps_rev  != null && el('motor_tilt_steps_rev'))  el('motor_tilt_steps_rev').value  = cfg.tilt_steps_rev;
    if (cfg.slider_steps_mm != null && el('motor_slider_steps_mm')) el('motor_slider_steps_mm').value = cfg.slider_steps_mm;
    if (cfg.focus_steps_mm  != null && el('motor_focus_steps_mm'))  el('motor_focus_steps_mm').value  = cfg.focus_steps_mm;
    _updateMotorDerivedLabels({
        pan_steps_rev:  cfg.pan_steps_rev  || 1600,
        tilt_steps_rev: cfg.tilt_steps_rev || 1600,
    });
}
...
function sendMotionSettings() {
    ...
    sendCmd('set_motion_roi', {
        ...
        warmup:         10,   // hidden — LK uses internal min_frames tracking confirmation
        cooldown:       parseFloat(document.getElementById('motion_cooldown')?.value  ?? 2.0),
        shutter_lag_ms: parseInt(document.getElementById('motion_shutter_lag')?.value ?? 80),
    });
}
...
function onCameraChange(val, isInit = false) {
    // Aperture control only for Sony cameras (gphoto2 or WiFi API)
    const isSony = val === 'sony' || val === 'sony_usb' || val === 'sony_s2';
    if (els.apertureRow) els.apertureRow.style.display = isSony ? '' : 'none';
    ...
    // Update PiP preview when camera changes
    if (currentMode === 'cinematic') {
        updatePiPPreview();
    }

    // On init restore, just refresh the feed URL — no WS command needed since
    // the server already knows its state.
    if (isInit) {
        els.mjpegFeed.src = `/video_feed?t=${Date.now()}`;
        return;
    }

    // Send set_camera to the server FIRST so it updates preview_camera before
    // the browser reconnects to /video_feed (avoids a race where the HTTP
    // reconnect arrives before the WS message and gets the wrong generator).
    sendCmd('set_camera', val);
    // Delay the feed reconnect slightly to ensure the WS message is processed.
    setTimeout(() => { els.mjpegFeed.src = `/video_feed?t=${Date.now()}`; }, 150);
...
function handleIncomingData(data) {
    ...
    if (data.type === "init") {
        ...
        // Exposure mode
        if (data.exp_mode) {
            _expMode = data.exp_mode;
            const radio = document.querySelector(`input[name="exp_mode"][value="${data.exp_mode}"]`);
            if (radio) radio.checked = true;
            _expModeApplyUI(data.exp_mode);
        }

        // Aperture slider
        if (data.picam_aperture_f != null && els.apertureSlider) {
            const idx = APERTURE_VALS.reduce((best, v, i) =>
                Math.abs(v - data.picam_aperture_f) < Math.abs(APERTURE_VALS[best] - data.picam_aperture_f) ? i : best, 0);
            els.apertureSlider.value = idx;
            if (els.apertureLabel) els.apertureLabel.innerText = `f/${APERTURE_VALS[idx]}`;
        }

        // Motor driver config (MKS servo42D steps-per-unit)
        _applyMotorConfigFromState(data);
    ...
    if (data.type === "hg_frame_estimate") {
        ...
        if (data.error || data.total == null) {
            out.innerText = `≈ ? frames  (${data.duration_hrs ?? '?'} hrs)`;
        } else {
            const b = data.breakdown || {};
            const detail = [
                b.day      ? `day ${b.day}`      : '',
                b.golden   ? `golden ${b.golden}` : '',
                b.twilight ? `twi ${b.twilight}`  : '',
                b.night    ? `night ${b.night}`   : '',
            ].filter(Boolean).join(' · ');
            out.innerText = `≈ ${data.total.toLocaleString()} frames  (${data.duration_hrs} hrs)`;
            out.title = detail;
            // Sync total_frames input so getSeqConfig() sends the right value
            const tf = document.getElementById('total_frames');
            if (tf) tf.value = data.total;
        }
    }
    ...
    if (data.type === "object_tracking_status") { handleObjectTrackingStatus(data); return; }
    if (data.type === "motion_lk_status")       { handleMotionLkStatus(data);       return; }
...
// ── Sony shutter speed list (1/3-stop increments, same as Sony camera menu) ──
const SONY_SHUTTERS = [
    // Long exposures (seconds)
    [30, '30"'], [25, '25"'], [20, '20"'], [15, '15"'], [13, '13"'], [10, '10"'],
    [8,  '8"'],  [6,  '6"'],  [5,  '5"'],  [4,  '4"'],  [3.2, '3.2"'], [2.5, '2.5"'],
    [2,  '2"'],  [1.6, '1.6"'], [1.3, '1.3"'], [1, '1"'],
    [0.8, '0.8"'], [0.6, '0.6"'], [0.5, '0.5"'], [0.4, '0.4"'], [0.3, '0.3"'],
    // Fractions
    [1/4,    '1/4'],   [1/5,    '1/5'],   [1/6,    '1/6'],   [1/8,    '1/8'],
    [1/10,   '1/10'],  [1/13,   '1/13'],  [1/15,   '1/15'],  [1/20,   '1/20'],
    [1/25,   '1/25'],  [1/30,   '1/30'],  [1/40,   '1/40'],  [1/50,   '1/50'],
    [1/60,   '1/60'],  [1/80,   '1/80'],  [1/100,  '1/100'], [1/125,  '1/125'],
    [1/160,  '1/160'], [1/200,  '1/200'], [1/250,  '1/250'], [1/320,  '1/320'],
    [1/400,  '1/400'], [1/500,  '1/500'], [1/640,  '1/640'], [1/800,  '1/800'],
    [1/1000, '1/1000'],[1/1250, '1/1250'],[1/1600, '1/1600'],[1/2000, '1/2000'],
...
function populateShutterSelect(id, defaultVal) {
    ...
    SONY_SHUTTERS.forEach(([val, label], i) => {
        const opt = document.createElement('option');
        opt.value = val;
        opt.textContent = label;
        sel.appendChild(opt);
        const diff = Math.abs(val - defaultVal);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Attach object tracking click handler to the feed container
    // Use event delegation because the MJPEG image gets recreated constantly
    const feedContainer = document.getElementById('feedContainer');
    if (feedContainer) {
        feedContainer.addEventListener('click', function(event) {
            // Only process clicks on the image itself, not on overlay or other elements
            const target = event.target;
            const img = document.getElementById('mjpegFeed');

            // Click is on the image, or click bubbled up from image through container
            if (target === img || target.parentElement === feedContainer) {
                // Create a synthetic event that looks like it came from the image
                const syntheticEvent = {
                    target: img || target,
                    clientX: event.clientX,
                    clientY: event.clientY
                };
                console.log('🖱️ Preview click detected, processing...');
                onPreviewClick(syntheticEvent);
            }
        });
        console.log('✓ Click handler attached to feedContainer');
    }
});
...
const overlay = {
    ...
    // LK motion detection overlay state (updated by motion_lk_status WS message)
    lkMotion: {
        lkState:        'watching',  // 'watching' | 'tracking' | 'armed' | 'triggered'
        trackingFrames: 0,
        minFrames:      5,
        subject:        null,        // [x, y] normalized or null
        velocity:       [0, 0],      // [vx, vy] normalized per frame
        target:         [0.5, 0.5],  // [x, y] normalized
        tClosestMs:     null,        // ms to closest approach (neg = receding)
        leadTimeMs:     null,        // ms of computed lead time
        distancePct:    null,        // % frame distance from subject to target
        approaching:    false,
        speedPct:       null,        // speed in % frame per frame
        predictedPos:   null,        // [x,y] predicted position at fire moment
    },
};
...
function setupCanvasOverlay() {
    ...
    // Apply initial button state to match _loupeUserVisible = false
    overlay.loupe.visible = false;
    const loupeBtn = document.getElementById('loupeToggleBtn');
    if (loupeBtn) {
        loupeBtn.style.opacity     = '0.4';
        loupeBtn.style.borderColor = '#444';
        loupeBtn.style.color       = '#555';
        loupeBtn.title             = 'Show focus loupe';
    }
...
function drawOverlay() {
    ...
    // ── Draw LK motion detection markers (timelapse predictive trigger) ─────
    const lkm = overlay.lkMotion;
    if (overlay.roi.visible && lkm.subject !== null) {
        const [tx, ty]   = lkm.target;
        const [sx, sy]   = lkm.subject;
        const [vx, vy]   = lkm.velocity;
        const tpx = tx * cw, tpy = ty * ch;
        const spx = sx * cw, spy = sy * ch;
        const st  = lkm.lkState;

        // Target crosshair — white ring + crosshairs
        ctx.save();
        ...
        ctx.beginPath(); ctx.arc(tpx, tpy, arm, 0, Math.PI * 2); ctx.stroke();
        ctx.restore();

        // Subject circle — yellow while locking, cyan when armed/triggered
        const subjectColor = (st === 'armed' || st === 'triggered') ? '#00ccff' : '#ffdd00';
        ctx.save();
        ctx.strokeStyle = subjectColor;
        ctx.lineWidth   = 2;
        ctx.beginPath(); ctx.arc(spx, spy, 13, 0, Math.PI * 2); ctx.stroke();

        // Velocity arrow — scale: 0.1 norm/frame → ~60px at 640w
        const velScale = 600;
        const ex = spx + vx * velScale * cw;
        const ey = spy + vy * velScale * ch;
        if (Math.abs(ex - spx) > 2 || Math.abs(ey - spy) > 2) {
            const angle = Math.atan2(ey - spy, ex - spx);
        ...
        // Predicted fire position (orange dot + dashed line from subject)
        if (lkm.predictedPos) {
            const ppx = lkm.predictedPos[0] * cw;
            const ppy = lkm.predictedPos[1] * ch;
            ctx.save();
            ctx.strokeStyle = 'rgba(255,140,0,0.7)';
            ctx.lineWidth   = 1.5;
            ctx.setLineDash([4, 4]);
            ctx.beginPath(); ctx.moveTo(spx, spy); ctx.lineTo(ppx, ppy); ctx.stroke();
            ctx.setLineDash([]);
        ...
        // Distance line from subject to target (thin, dimmed)
        ctx.save();
        ctx.strokeStyle = 'rgba(255,255,255,0.2)';
        ctx.lineWidth   = 1;
        ctx.setLineDash([3, 6]);
        ctx.beginPath(); ctx.moveTo(spx, spy); ctx.lineTo(tpx, tpy); ctx.stroke();
        ctx.setLineDash([]);
        ctx.restore();

        // ARMED / TRIGGERED pulse ring around target
        if (st === 'armed' || st === 'triggered') {
            ctx.save();
            const pulse = (st === 'triggered') ? 'rgba(0,255,136,0.9)' : 'rgba(0,200,255,0.8)';
            ctx.strokeStyle = pulse;
            ctx.lineWidth   = 2.5;
            ctx.beginPath(); ctx.arc(tpx, tpy, 22, 0, Math.PI * 2); ctx.stroke();
            ctx.restore();
        }

        // Confidence arc around subject (sweeps to full as lock count climbs)
        if (st === 'tracking' || st === 'armed') {
            const frac = Math.min(1, lkm.trackingFrames / Math.max(1, lkm.minFrames));
            ctx.save();
            ctx.strokeStyle = subjectColor;
            ctx.lineWidth   = 3;
            ctx.beginPath();
            ctx.arc(spx, spy, 19, -Math.PI / 2, -Math.PI / 2 + frac * Math.PI * 2);
            ctx.stroke();
            ctx.restore();
        }

        // t_closest countdown label near target (when tracking or armed)
        if ((st === 'tracking' || st === 'armed') && lkm.tClosestMs !== null) {
            const approaching = lkm.approaching;
            const tLabel = approaching
                ? `${lkm.tClosestMs}ms`
                : `away`;
            ctx.save();
            ctx.font      = 'bold 11px monospace';
            ctx.fillStyle = approaching ? subjectColor : 'rgba(200,200,200,0.6)';
            ctx.textAlign = 'center';
            ctx.fillText(tLabel, tpx, tpy - 26);
            ctx.textAlign = 'left';
            ctx.restore();
        }
    }
...
function macroCalc() {
    ...
    const slotC = document.getElementById('macro_slot_c_enabled')?.checked ? 1 : 0;
...
function handleMacroProgress(data) {
    ...
    const done  = data.total_stacks && data.total_frames
        ? Math.max(0, (data.stack - 1)) * data.total_frames + (data.frame || 0)
        : 0;
    const total = data.total_stacks && data.total_frames
        ? data.total_stacks * data.total_frames : 0;
    const pct = total ? Math.min(100, Math.round(done / total * 100)) : 0;
    const bar = document.getElementById('macro_prog_bar');
    if (bar) bar.style.width = pct + '%';

    // ── Main "SEQUENCE PROGRESS" display (always visible) ────────────────────
    // Global frame number across all stacks so the big counter makes sense.
    if (els.curFrame) els.curFrame.innerText = String(done).padStart(3, '0');
    if (els.totFrame) els.totFrame.innerText = total || '—';
    if (els.progressMsg) {
        const stackLabel = data.total_stacks
            ? `Stack ${data.stack}/${data.total_stacks}  Frame ${data.frame ?? 0}/${data.total_frames ?? '—'}`
            : (data.msg || 'Macro scan running…');
        els.progressMsg.innerText = stackLabel;
    }
    // Mirror pan/tilt/rail into the slider position readouts if elements exist
    if (data.pan_deg  !== undefined && els.valP) els.valP.innerText = data.pan_deg.toFixed(1) + '°';
    if (data.rail_mm  !== undefined && els.valS) els.valS.innerText = data.rail_mm.toFixed(2) + ' mm';
    if (data.tilt_deg !== undefined && els.valT) els.valT.innerText = data.tilt_deg.toFixed(1) + '°';
}
...
function handleMacroStackComplete(data) {
    ...
    log(`✓ Stack ${data.stack}/${data.total_stacks} complete — pan ${data.pan_deg?.toFixed(1)}°  tilt ${data.tilt_deg?.toFixed(1) ?? '—'}°`);
}
...
function handleMacroDone(data) {
    ...
    // Reset main progress display
    if (els.curFrame)   els.curFrame.innerText  = '000';
    if (els.progressMsg) els.progressMsg.innerText = data.interrupted ? '⚠ Macro stopped.' : '✓ Macro complete.';
...
function macroStart() {
    ...
    const payload = {
        ...
        slots: [
            ...
            {
                id: 'slot_C',
                label: document.getElementById('macro_slot_c_label')?.value || 'grey bg',
                enabled: document.getElementById('macro_slot_c_enabled')?.checked ?? false,
                relay1: document.getElementById('macro_slot_c_relay1')?.checked ?? false,
                relay2: document.getElementById('macro_slot_c_relay2')?.checked ?? false,
                relay_settle_ms: parseInt(document.getElementById('macro_slot_c_settle')?.value || 0),
                relay_release_ms: parseInt(document.getElementById('macro_slot_c_release')?.value || 0),
                iso: parseInt(document.getElementById('macro_slot_c_iso')?.value || 400),
                shutter_s: parseFloat(document.getElementById('macro_slot_c_shutter')?.value || 0.008),
                kelvin: parseInt(document.getElementById('macro_slot_c_kelvin')?.value || 5500),
...
function captureFlats() {
    // Build slot list from current UI state (same as macroStart slots)
    const slots = [
        {
            id: 'slot_A', enabled: true,
            iso:       parseInt(document.getElementById('macro_slot_a_iso')?.value || 200),
            shutter_s: parseFloat(document.getElementById('macro_slot_a_shutter')?.value || 0.033),
            kelvin:    parseInt(document.getElementById('macro_slot_a_kelvin')?.value || 5500),
            ae:        document.getElementById('macro_slot_a_ae')?.checked ?? false,
            bg_color:  document.getElementById('macro_slot_a_bg')?.value || '',
            bg_settle_ms: parseInt(document.getElementById('macro_slot_a_bg_settle')?.value || 300),
    ...
    if (slots.length === 0) {
        log('⚠ No enabled slots with a BG color — nothing to capture.');
        return;
    }
...
function showBgPhoneQR() {
    ...
    img.src = qrSrc;
    _updateBgPhoneStatus();
    modal.style.display = 'flex';
}

function _updateBgPhoneStatus() {
    const el = document.getElementById('bgPhoneStatus');
    if (!el) return;
    if (_bgPhoneCount > 0) {
        el.style.color = '#4caf50';
        el.textContent = `● ${_bgPhoneCount} phone${_bgPhoneCount > 1 ? 's' : ''} connected`;
    } else {
        el.style.color = '#888';
        el.textContent = '● No phone connected';
    }
}

// Called from the WebSocket message handler when the server broadcasts bg_phone_state
function handleBgPhoneState(count) {
    _bgPhoneCount = count;
    _updateBgPhoneStatus();
}
...
let _objectTrackingEnabled = false;
let _trackingDragStart = null;
let _trackingSelection = null;  // {x1, y1, x2, y2} in normalized coords [0,1]
let _trackingStatus = { active: false, pan: 0, tilt: 0, message: '' };

function updateTrackingGainLabel(key) {
    const el = document.getElementById(`track_${key}`);
    const lbl = document.getElementById(`track_${key}_val`);
    if (el && lbl) lbl.textContent = parseFloat(el.value).toFixed(2);
}

function sendTrackingGains() {
    sendCmd('set_tracking_gains', {
        kv:  parseFloat(document.getElementById('track_kv')?.value  ?? 1.2),
        kp:  parseFloat(document.getElementById('track_kp')?.value  ?? 0.8),
        kd:  parseFloat(document.getElementById('track_kd')?.value  ?? 0.4),
        kff: parseFloat(document.getElementById('track_kff')?.value ?? 0.0),
        ki:  parseFloat(document.getElementById('track_ki')?.value  ?? 0.0),
    });
}

function toggleObjectTracking() {
    if (_objectTrackingEnabled) {
        sendCmd('stop_object_tracking');
        _trackingSelection = null;
        clearTrackingOverlay();
    } else {
        sendCmd('start_object_tracking');
        log("Object tracking enabled — drag on preview to select tracking area");
        drawTrackingActive();
    }
}

function drawTrackingActive() {
...
function drawTrackingFeedback() {
    // Called periodically to show real-time pan/tilt feedback
    if (!_objectTrackingEnabled || !_trackingSelection) {
        return;
    }

    const container = document.getElementById('feedContainer');
    const greenCrosshair = document.getElementById('greenCrosshair');

    if (!container || !greenCrosshair) return;

    const rect = container.getBoundingClientRect();
    const w = rect.width;
    const h = rect.height;

    // Green crosshair follows detected object position from backend
    // Use _trackingStatus.detected if available, otherwise use selection center
    let greenCx, greenCy;

    if (_trackingStatus && _trackingStatus.detected && Array.isArray(_trackingStatus.detected)) {
        // Backend provided actual detected position
        const detX = _trackingStatus.detected[0];  // normalized [0, 1]
        const detY = _trackingStatus.detected[1];  // normalized [0, 1]
        greenCx = detX * w;
        greenCy = detY * h;
        console.log('🟢 Green at detected:', { detX: detX.toFixed(3), detY: detY.toFixed(3), px: greenCx.toFixed(0), py: greenCy.toFixed(0) });
    } else {
        // Fallback to selection center if no detection yet
        greenCx = (((_trackingSelection.x1 + _trackingSelection.x2) / 2) * w);
        greenCy = (((_trackingSelection.y1 + _trackingSelection.y2) / 2) * h);
        console.log('🟢 Green at selection center (no backend detection yet)');
    }

    greenCrosshair.style.left = greenCx + 'px';
    greenCrosshair.style.top = greenCy + 'px';
    greenCrosshair.style.display = 'block';

    // Blue crosshair stays at fixed position (set during selection)
...
function processTrackingSelection(normX, normY) {
    try {
        log(`✓ Selected: ${(normX * 100).toFixed(0)}%, ${(normY * 100).toFixed(0)}%`);

        const boxSize = 0.12;
        _trackingSelection = {
            x1: Math.max(0, normX - boxSize / 2),
            y1: Math.max(0, normY - boxSize / 2),
            x2: Math.min(1, normX + boxSize / 2),
            y2: Math.min(1, normY + boxSize / 2),
        };

        sendCmd('select_tracking_region', { x: normX, y: normY });
        drawTrackingSelectionBox(_trackingSelection);
    } catch (err) {
        log('❌ Error: ' + err.message);
        console.error('processTrackingSelection error:', err);
    }
}

function onPreviewClick(event) {
    try {
        console.log('🎯 onPreviewClick called!', { tracking: _objectTrackingEnabled });
        log('🎯 Click detected!');

        if (!_objectTrackingEnabled) {
            log('⚠️ Enable tracking first');
            return;
        }
        ...
        processTrackingSelection(normX, normY);
    } catch (err) {
        log('❌ Click error: ' + err.message);
        console.error('onPreviewClick error:', err);
    }
}


function drawTrackingSelectionBox(sel) {
    const container = document.getElementById('feedContainer');
    const greenCrosshair = document.getElementById('greenCrosshair');
    const blueCrosshair = document.getElementById('blueCrosshair');

    if (!container || !greenCrosshair || !blueCrosshair) return;

    const rect = container.getBoundingClientRect();
    const w = rect.width;
    const h = rect.height;

    // Blue crosshair at center of selection (where user clicked for desired composition)
    const blueCx = ((sel.x1 + sel.x2) / 2) * w;
    const blueCy = ((sel.y1 + sel.y2) / 2) * h;

    blueCrosshair.style.left = blueCx + 'px';
    blueCrosshair.style.top = blueCy + 'px';
    blueCrosshair.style.display = 'block';

    // Green crosshair same position initially
    greenCrosshair.style.left = blueCx + 'px';
    greenCrosshair.style.top = blueCy + 'px';
    greenCrosshair.style.display = 'block';
}

function updateGreenCrosshair(detectedPos) {
    // Update green crosshairs to follow detected object position
    if (!detectedPos || !Array.isArray(detectedPos)) return;
    ...
    const detX = detectedPos[0];  // normalized [0, 1]
    const detY = detectedPos[1];  // normalized [0, 1]
    const greenCx = detX * w;
    const greenCy = detY * h;

    greenCrosshair.style.left = greenCx + 'px';
    greenCrosshair.style.top = greenCy + 'px';
    greenCrosshair.style.display = 'block';

    console.log('🟢 Green crosshair updated:', { detX: detX.toFixed(3), detY: detY.toFixed(3), px: greenCx.toFixed(0), py: greenCy.toFixed(0) });
}

let _trackingFeedbackInterval = null;

function updateBlueCrosshair(normX, normY) {
    // Move the blue (target) crosshair to a new normalized frame position.
    // Called when the backend reports the composition target shifted (e.g. via joystick).
    const container = document.getElementById('feedContainer');
    const blueCrosshair = document.getElementById('blueCrosshair');
    if (!container || !blueCrosshair) return;
    const rect = container.getBoundingClientRect();
    blueCrosshair.style.left = (normX * rect.width)  + 'px';
    blueCrosshair.style.top  = (normY * rect.height) + 'px';
    blueCrosshair.style.display = 'block';
}

function handleObjectTrackingStatus(data) {
    const badge = document.getElementById('tracking_status_badge');
    const activeRow = document.getElementById('tracking_active_row');
    const btn = document.getElementById('tracking_toggle_btn');

    _objectTrackingEnabled = data.active;
    _trackingStatus = data;  // Store status for feedback drawing

    // Update green crosshairs immediately with detected object position
    if (data.active && data.detected && _trackingSelection) {
        updateGreenCrosshair(data.detected);
    }

    // Update blue crosshair when backend sends a shifted composition target
    // (happens when user moves pan/tilt joystick while tracking)
    if (data.active && data.target && Array.isArray(data.target)) {
        updateBlueCrosshair(data.target[0], data.target[1]);
    }

    if (badge) {
        badge.innerText = data.active ? 'ACTIVE' : 'OFF';
        badge.style.color = data.active ? 'var(--accent-teal)' : 'var(--text-muted)';
        badge.style.borderColor = data.active ? 'var(--accent-teal)' : '#333';
    }

    if (btn) {
        btn.textContent = data.active ? '⊘ Stop Tracking' : '🎯 Enable Tracking';
        btn.style.background = data.active ? '#8b3a3a' : 'var(--accent-teal)';
        btn.style.borderColor = data.active ? '#b85454' : 'var(--accent-teal)';
    }

    if (activeRow) {
        if (data.active) {
            activeRow.style.display = 'block';
            const msg = document.getElementById('tracking_active_msg');
            if (msg) msg.innerText = data.message || '✓ Tracking active — following subject';
            drawTrackingActive();
            // Start drawing feedback at 10 Hz
            if (!_trackingFeedbackInterval) {
                _trackingFeedbackInterval = setInterval(drawTrackingFeedback, 100);
            }
        } else {
            activeRow.style.display = 'none';
            clearTrackingOverlay();
            if (_trackingFeedbackInterval) {
                clearInterval(_trackingFeedbackInterval);
                _trackingFeedbackInterval = null;
            }
        }
    }
}

// ── LK Motion Trigger Status ──────────────────────────────────────────────────
function handleMotionLkStatus(data) {
    // Update canvas overlay state
    const lkm = overlay.lkMotion;
    lkm.lkState        = data.lk_state       || 'watching';
    lkm.trackingFrames = data.tracking_frames || 0;
    lkm.minFrames      = data.min_frames      || 5;
    lkm.subject        = data.subject         || null;
    lkm.velocity       = data.velocity        || [0, 0];
    lkm.target         = data.target          || [0.5, 0.5];
    lkm.tClosestMs     = data.t_closest_ms    ?? null;
    lkm.leadTimeMs     = data.lead_time_ms    ?? null;
    lkm.distancePct    = data.distance_pct    ?? null;
    lkm.approaching    = data.approaching     || false;
    lkm.speedPct       = data.speed_pct       ?? null;
    lkm.predictedPos   = data.predicted_pos   || null;

    // Dummy fire flash — fires even when no sequence is running
    if (data.would_fire) _motionDummyFire();

    // ── Update HTML status elements ───────────────────────────────────────────
    const dot      = document.getElementById('motion_lk_dot');
    const label    = document.getElementById('motion_lk_label');
    const bar      = document.getElementById('motion_lk_bar');
    const readout  = document.getElementById('motion_lk_readout');

    if (!dot || !label || !bar) return;

    const st     = lkm.lkState;
    const frames = lkm.trackingFrames;
    const minF   = lkm.minFrames;
    ...
    if (st === 'triggered') {
        ...
        const t = lkm.tClosestMs;
        const tStr = (t !== null && t > 0) ? `fires in ${t}ms` : 'will fire';
        dot.style.background = '#00ccff';
        dot.style.boxShadow  = '0 0 6px #00ccff';
        label.textContent    = `◎ ARMED — ${tStr}`;
        label.style.color    = '#00ccff';
        bar.style.width      = '100%';
        bar.style.background = '#00ccff';

    } else if (st === 'tracking') {
        const approach = lkm.approaching
            ? (lkm.tClosestMs !== null ? `→ ${lkm.tClosestMs}ms to target` : '→ approaching')
            : '← moving away';
        dot.style.background = '#ffdd00';
        dot.style.boxShadow  = '0 0 4px #ffdd00';
        label.textContent    = `◑ Locking (${frames}/${minF}) — ${approach}`;
        label.style.color    = '#ffdd00';
        bar.style.width      = Math.min(100, Math.round(frames / minF * 100)) + '%';
        bar.style.background = '#ffdd00';

    } else {
        dot.style.background = '#555';
        dot.style.boxShadow  = 'none';
        label.textContent    = '○ Watching for motion';
        label.style.color    = 'var(--text-dim)';
        bar.style.width      = '0%';
        bar.style.background = '#555';
    }

    // ── Readout row: distance · speed · lead time ─────────────────────────────
    if (readout) {
        if (lkm.subject && lkm.distancePct !== null) {
            const dist  = lkm.distancePct.toFixed(1);
            const speed = lkm.speedPct !== null ? lkm.speedPct.toFixed(2) : '—';
            const lead  = lkm.leadTimeMs !== null ? `${lkm.leadTimeMs}ms` : '—';
            const dir   = lkm.approaching ? '↗' : '↙';
            readout.textContent = `${dir} dist ${dist}%  ·  speed ${speed}%/f  ·  lead ${lead}`;
...
// ── Dummy fire flash (setup preview — shows when trigger WOULD have fired) ────
let _dummyFireTimeout = null;
function _motionDummyFire() {
    // Don't flash if a real sequence is running — that would fire for real
    if (isRunning) return;

    const feed = document.getElementById('feedContainer');
    const dot  = document.getElementById('motion_lk_dot');
    const lbl  = document.getElementById('motion_lk_label');
    if (!feed) return;

    // Flash the video feed border orange
    feed.classList.add('motion-dummy-fire');

    // Flash the status label
    if (lbl) {
        lbl.textContent  = '⚡ WOULD FIRE';
        lbl.style.color  = '#ff9900';
    }
    if (dot) {
        dot.style.background = '#ff9900';
        dot.style.boxShadow  = '0 0 10px #ff9900';
    }

    // Clear existing timeout so rapid calls don't stack
    if (_dummyFireTimeout) clearTimeout(_dummyFireTimeout);
    _dummyFireTimeout = setTimeout(() => {
        feed.classList.remove('motion-dummy-fire');
        _dummyFireTimeout = null;
        // Status will be corrected on next broadcast (~100ms)
    }, 600);
}

// ── Picture-in-Picture Preview ────────────────────────────────────────────────
let _pipRefreshInterval = null;

function updatePiPPreview() {
    const activeCamera = document.getElementById('camera_select')?.value || 'picam';
    const pipContainer = document.getElementById('pipContainer');
    const pipFeed = document.getElementById('pipFeed');
    const pipLabel = document.getElementById('pipLabel');

    if (!pipContainer || !pipFeed) return;

    // Show PiP if using Sony for recording while tracking with Pi camera, or vice versa
    let showPiP = false;
    let pipCameraName = '';
    let pipSource = '';

    if (activeCamera === 'picam') {
        // Viewing Pi camera - show Sony as PiP if available
        const cameraSelect = document.getElementById('camera_select');
        const options = cameraSelect?.querySelectorAll('option');
        let hasSony = false;
        options?.forEach(opt => {
            if (opt.value.includes('sony')) hasSony = true;
        });
        if (hasSony) {
            showPiP = true;
            pipCameraName = 'Sony';
            pipSource = '/video_feed?camera=sony&t=' + Date.now();
        }
    } else if (activeCamera.includes('sony')) {
        // Viewing Sony - show Pi camera as PiP for tracking reference
        showPiP = true;
        pipCameraName = 'Pi Cam';
        pipSource = '/video_feed?camera=picam&t=' + Date.now();
    }

    if (showPiP) {
        pipContainer.style.display = 'block';
        pipFeed.src = pipSource;
        if (pipLabel) pipLabel.innerText = pipCameraName;
    } else {
...
function startPiPRefresh() {
    stopPiPRefresh();
    updatePiPPreview();
    // Refresh PiP every 2 seconds
    _pipRefreshInterval = setInterval(updatePiPPreview, 2000);
}

function stopPiPRefresh() {
    if (_pipRefreshInterval) {
        clearInterval(_pipRefreshInterval);
        _pipRefreshInterval = null;
    }
}
...
// ─── CAMERA SETTINGS READOUT (camera mode) ───────────────────────────────────
function handleCameraSettingsReadout(data) {
    const el = els.camSettingsReadout;
    if (!el) return;
    const parts = [];
    if (data.shutter != null) parts.push(`Shutter: ${prettyShutter(data.shutter)}`);
    if (data.iso    != null) parts.push(`ISO: ${data.iso}`);
    if (data.aperture != null) parts.push(`f/${data.aperture}`);
    if (data.kelvin != null) parts.push(`WB: ${data.kelvin}K`);
    if (data.error) {
        el.innerHTML = `<span style="color:var(--accent-red)">⚠ ${data.error}</span>`;
    } else {
        el.innerHTML = parts.map(p => `<span class="cam-readout-chip">${p}</span>`).join('');
    }
}

// ─── PREVIEW SHOT MODAL ───────────────────────────────────────────────────────
function showPreviewModal(url) {
    let modal = document.getElementById('previewShotModal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'previewShotModal';
        modal.style.cssText = `
            position:fixed; inset:0; z-index:9999;
        ...
        `;
        modal.innerHTML = `
            <img id="previewShotImg" style="max-width:95vw; max-height:88vh; object-fit:contain; border-radius:4px; box-shadow:0 4px 32px #000;">
            <button onclick="document.getElementById('previewShotModal').remove()"
                style="padding:8px 28px; font-size:0.9rem; background:var(--surface); color:var(--text);
    ...
    const img = document.getElementById('previewShotImg');
