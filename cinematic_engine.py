...
RIG_PRESETS = {
    #                 slider_mass  drag   pt_scale  pt_mass
    #                 ──────────  ─────  ────────  ───────
    # pt_mass is the pan/tilt deceleration mass used during joystick cinema mode.
    # Higher than slider_mass so pan/tilt ramp down more slowly, giving the
    # MKS servo42D closed-loop time to stop without mechanical overshoot/hunting.
    "responsive": (0.05, 0.95, 1.00, 0.10),
    "light":      (0.15, 0.80, 1.00, 0.40),
    "standard":   (0.40, 0.55, 1.00, 0.90),  # pan/tilt τ ≈ 1.6 s vs slider τ ≈ 0.7 s
    "heavy":      (0.90, 0.30, 1.00, 1.80),
...
class InertiaEngine:
    ...
    def __init__(self, hardware, guard: SoftLimitGuard, broadcast_fn,
        ...
        # Physics params
        self.mass:    float = 0.40   # slider deceleration mass
        self.pt_mass: float = 0.90   # pan/tilt deceleration mass (higher → smoother stop)
        self.drag:    float = 0.55   # viscosity coefficient (shared)
        ...
        self._current_preset: str = "standard"     # updated by set_preset()
        # When object tracking is active, pan/tilt use these overrides instead of
        # self.mass / self.drag so the slider keeps its heavy cinematic feel while
        # pan/tilt respond quickly enough for stable closed-loop tracking.
        # Set to None to use the global values (normal joystick / keyframe mode).
        self.pan_tilt_mass_override: Optional[float] = None
        self.pan_tilt_drag_override: Optional[float] = None
    ...
    def set_preset(self, name: str, apply_pt_scale: bool = True):
        ...
        if name in RIG_PRESETS and RIG_PRESETS[name] is not None:
            mass, drag, pt_scale, pt_mass = RIG_PRESETS[name]
            self.mass    = mass
            self.pt_mass = pt_mass
            self.drag    = drag
            self._current_preset = name   # track for restore-after-tracking
            if apply_pt_scale:
                self.pan_tilt_scale = pt_scale
            logger.info(
                f"Inertia preset: {name} "
                f"(mass={self.mass}, pt_mass={self.pt_mass}, drag={self.drag}, "
                f"pt_scale={self.pan_tilt_scale:.2f})"
            )
    ...
    def _thread_loop(self):
        """50 Hz physics + VACTUAL motor command loop — runs in a dedicated thread.

        Isolating this from the asyncio event loop means camera captures, WebSocket
        broadcasts, and other async I/O can never delay a motor tick.  Real-time
        scheduling (SCHED_FIFO) ensures the OS won't preempt this thread for normal
        user-space work, giving sub-millisecond tick jitter instead of the 2–5 ms
        jitter that asyncio.sleep() produces under event-loop load.
        """
        # Request real-time scheduling — runs as root so this will succeed.
        try:
            param = os.sched_param(50)   # priority 50/99 — high but below kernel threads
            os.sched_setscheduler(0, os.SCHED_FIFO, param)
            logger.info("InertiaEngine: SCHED_FIFO priority 50 set.")
        except Exception as _sched_err:
            logger.warning(f"InertiaEngine: could not set SCHED_FIFO ({_sched_err}) — running at normal priority.")
    ...
    def _tick(self):
        ...
        # exponent < 1 boosts small joystick deflections for precision near zero.
        def _map(t: float, max_v: float) -> float:
            sign = 1 if t >= 0 else -1
            return sign * (abs(t) ** 0.6) * max_v * self._speed_mult

        # Linear mapping for algorithmic (tracking) input — no power-curve distortion.
        # Object tracking already shapes its output via tanh; the 0.6 power law would
        # amplify small error signals by 3–6× and create unpredictable step sizes.
        def _map_linear(t: float, max_v: float) -> float:
            return t * max_v * self._speed_mult

        # Software deadzone on the normalized stick target.
        # Prevents gamepad-drift creep on some controllers whose hardware deadzone
        # sits 1–3% outside center.  2% is safe for human sticks.
        # For tracking (algorithmic) input the deadzone is tightened to 0.1% so
        # small-but-real corrections aren't silently swallowed — the tracking control
        # law already zeroes commands inside its own 0.5% deadband.
        _pt_tracking = self.pan_tilt_mass_override is not None
        _DZ    = 0.02    # 2% — joystick drift guard
        _DZ_PT = 0.001   # 0.1% — pass-through for algorithmic tracking
        t_slider = self._t_slider if abs(self._t_slider) > _DZ    else 0.0
        t_pan    = self._t_pan    if abs(self._t_pan)    > (_DZ_PT if _pt_tracking else _DZ) else 0.0
        t_tilt   = self._t_tilt   if abs(self._t_tilt)   > (_DZ_PT if _pt_tracking else _DZ) else 0.0

        # Target velocities in physical units (mm/s, deg/s).
        # motion_scale (pan_tilt_scale) is applied AFTER the map so it acts as a
        # true linear multiplier — see comment in __init__ for why.
        # Slider always uses power-curve mapping (manual joystick control only).
        # Pan/tilt use linear mapping when tracking overrides are active.
        target_slider = _map(t_slider, self.guard.slider.speed_limit) * self.pan_tilt_scale
        if _pt_tracking:
            target_pan  = _map_linear(t_pan,  self.guard.pan.speed_limit)  * self.pan_tilt_scale
            target_tilt = _map_linear(t_tilt, self.guard.tilt.speed_limit) * self.pan_tilt_scale
        else:
        ...
        # Physics: dv/dt = (target - drag*v) / mass  →  τ = mass/drag
        # Slider always uses self.mass / self.drag (user's chosen preset).
        # Pan/tilt use override values when set (object tracking) so the slider
        # keeps its heavy cinematic feel while pan/tilt stay lag-free.
        # τ must be ≪ control period (50 ms) for stable closed-loop tracking.
        dt = INERTIA_DT
        inv_mass_s  = 1.0 / max(0.01, self.mass)
        # Tracking override takes priority; fall back to per-axis pt_mass (not slider mass)
        # so pan/tilt decelerate more gradually than the slider in joystick cinema mode.
        _pt_mass    = self.pan_tilt_mass_override if self.pan_tilt_mass_override is not None else self.pt_mass
        _pt_drag    = self.pan_tilt_drag_override if self.pan_tilt_drag_override is not None else self.drag
        inv_mass_pt = 1.0 / max(0.01, _pt_mass)

        self._v_slider += (target_slider - self.drag  * self._v_slider) * inv_mass_s  * dt
        self._v_pan    += (target_pan    - _pt_drag   * self._v_pan)    * inv_mass_pt * dt
        self._v_tilt   += (target_tilt   - _pt_drag   * self._v_tilt)   * inv_mass_pt * dt
