...
class HardwareController:
    ...
    def __init__(self, gpio_chip_index: int = 4):  # Pi 5: main GPIO is gpiochip4
        ...
        try:
            ...
            # Velocity cache: tracks the last (freq, forward) sent per axis so
            # set_axis_speed() can skip lgpio.tx_pwm() when nothing changed.
            # lgpio.tx_pwm() restarts the waveform unconditionally — at low step
            # rates (VACTUAL 1–5 = 4–21 Hz) a restart every 20ms disrupts the
            # step stream before a single step can complete → audible crunch.
            # False = axis is stopped; (freq, forward) = axis is running.
            self._axis_speed_cache: dict = {}
    ...
    def move_axes_simultaneous(
        ...
        # Sync cache — we stopped the PWMs directly, bypassing set_axis_speed.
    ...
    def set_axis_speed(self, axis: str, steps_per_sec: float):
        ...
        lgpio.tx_pwm() restarts the hardware waveform unconditionally every time
        it is called, even when the frequency is unchanged.  At low step rates
        (VACTUAL 1–5 = 4–21 Hz during tracking corrections) the InertiaEngine
        calls this 50× per second — faster than a single step can complete at
        ~4 Hz.  Each restart resets the PWM phase, producing irregular step
        timing that sounds like crunch.  The velocity cache below skips the
        lgpio call entirely when neither frequency nor direction has changed,
        letting the existing waveform run uninterrupted.
        """
        ...
        # Cache key: False means "stopped", (freq, forward) means "running".
        # Empty dict on init means first call for any axis is always a miss.
        cache_key = False if stopping else (freq, forward)
        if self._axis_speed_cache.get(axis) == cache_key:
            return  # no change — let the existing waveform continue undisturbed

        self._axis_speed_cache[axis] = cache_key
    ...
    def stop_all_axes(self):
        ...
        # Sync cache so set_axis_speed knows all axes are now stopped.
        # (stop_all_axes calls lgpio directly, bypassing the normal path.)
        self._axis_speed_cache = {'slider': False, 'pan': False, 'tilt': False}
